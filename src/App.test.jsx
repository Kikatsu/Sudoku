...
describe("app startup", () => {
  it("starts new visitors on an easy free-training board", () => {
    const game = createInitialGame({ kind: "home", lessonId: null }, null);

    expect(game.mode).toBe("free");
    expect(game.difficulty).toBe("easy");
  });

  it("continues an existing saved board on the home route", () => {
    const loaded = createGame("hard", "daily");

    expect(createInitialGame({ kind: "home", lessonId: null }, loaded)).toBe(loaded);
  });
});
